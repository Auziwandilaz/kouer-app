...
import IconCarrot from "../components/svg/icon_carrot";
import { LinearGradient } from "expo-linear-gradient";
import { Rating } from "@kolking/react-native-rating";
import { theme_vegetables as theme } from "../lib/constants";
...
export default function VracVegetables({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      console.log("Début de l'appel RPC");
      console.log("Catégorie :", theme.category); // Afficher la catégorie

      try {
        const ret = await supabase.rpc("get_products_by_categories", {
          category_names: [theme.category],
        });

        console.log("Réponse complète de l'appel RPC :", ret); // Afficher la réponse complète

        if (ret.data) {
          const firstTwentyItems = ret.data.slice(0, 20);
          setData(firstTwentyItems);
          setDisplayData(firstTwentyItems);
        } else {
          console.log(
            "Erreur : les données attendues ne sont pas présentes dans la réponse"
          );
        }
      } catch (error) {
        console.log("Erreur lors de l'appel RPC :", error);
      }

      console.log("Fin de l'appel RPC");
    })();
  }, []);
    ...
    <SafeAreaView className="flex-1">
      <View className={`flex-1 ${showFilter ? "hidden" : ""}`}>
        <View className="h-[25vh]">
          ...
          >
            <IconCarrot className="w-12 mr-2" />
          ...
          <View className="w-full -mt-4 px-5">
            ...
            >
              ...
              <View className="absolute inset-y-0 right-0.5 justify-center">
                ...
                >
                  <IconSearch className="w-full h-full" color="white" />
                </LinearGradient>
              </View>
            </View>
          </View>
              ...
              <View className="flex-[0.5] h-[25vh] mx-[10] p-[5] rounded-[10px] justify-between bg-white">
                ...
                <View className="flex-row w-full justify-between">
                  ...
                  <TouchableOpacity activeOpacity={0.6}>
                    <IconShopBag className="w-8 h-8" color="#4EA04C" />
                  </TouchableOpacity>
                </View>
              </View>
            );
      ...
      <View className={`flex-1 ${showFilter ? "" : "hidden"}`}>
        <View className="flex 1">
          ...
          >
            ...
            <IconNext className="w-[20] h-1/3 mr-[5]" color="#505050" />
          </TouchableOpacity>
          ...
          >
            <View className="flex-row h-[50] pl-[5] border-b border-b-[#E3E3E3] bg-[#4EA04C]">
              <View className="flex-row w-full items-center justify-between bg-white">
                ...
                {searchCollapse ? (
                  <IconPlus className="w-[20] h-full mr-[5]" color="#505050" />
                ) : (
                  <IconMinus className="w-[20] h-full mr-[5]" color="#505050" />
                )}
              </View>
            </View>
          </TouchableWithoutFeedback>
          <Collapsible className="px-5" collapsed={searchCollapse}>
            <View
                ...
                return (
                  <View className="flex-row h-8 mt-1 ml-2 px-3 self-start items-center rounded-full border border-[#E3E3E3] bg-white">
                    ...
                    <TouchableOpacity
                      ...
                      onPress={(e) => {}}
                    >
                      <IconCross className="w-3 h-3 ml-2" color="#505050" />
                    </TouchableOpacity>
                  </View>
                );
              })}
          ...
          <Collapsible className="px-5" collapsed={originCollapse}>
            <View
              className={`flex-row w-full h-8 mt-5 ${
          ...
          <Collapsible collapsed={sortCollapse}>
            <View className="px-5 py-2">
              {Object.keys(sortFunctions).map((item) => {
                return (
          ...
          <Collapsible collapsed={priceCollapse}>
            <View className="px-5 py-2">
              {Object.keys(priceFilters).map((item) => {
                return (
                  <View className="flex-row w-full h-5 my-2 justify-between">
                    <Text className="text-[15px] text-[#505050]">{item}</Text>
                    <RadioButton
          ...
          >
            <View className="flex-row h-[50] pl-[5] border-b border-b-[#E3E3E3] bg-[#4EA04C]">
              <View className="flex-row w-full items-center justify-between bg-white">
                <View className="pl-[5]">
                  <Text className="text-xl text-[#505050]">
                    Note du produit
                  </Text>
          ...
          <Collapsible collapsed={ratingCollapse}>
            <View className="px-5 py-2">
              {Object.keys(ratingFilters).map((item) => {
                return (
                  <View className="flex-row w-full h-5 my-2 justify-between">
              ...
              onPress={() => {
                ...
                let tmp = data.filter((product) => {
                  ...
                  return priceTags.some((filter) =>
                    priceFilters[filter](product.product_unit_price)
                  );
                });
                tmp.sort(sortFunctions[sortFilter]);
                setDisplayData(tmp);
              }}
            >
              <LinearGradient
